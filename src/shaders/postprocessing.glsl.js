...
const CustomShader = {
	...
	fragmentShader: /* glsl */`
		...
		void main() {
            ...
            // vec2 cp = -1.0 + (2.0 * gl_FragCoord.xy / resolution.xy);
            // float cl = length(cp);
            // vec2 uv = gl_FragCoord.xy / resolution.xy + (cp / cl) * cos(cl * 12.0 - time * .1) * 0.02;
            // gl_FragColor = texture2D(tDiffuse, uv);//.xyz;
            ...
            //     vec2 cp = -1.0 + (2.0 * gl_FragCoord.xy / resolution.xy);
            //     float cl = length(cp);
            //     vec2 uv = (gl_FragCoord.xy / resolution.xy) + (cp / cl) * cos(cl * 1.0) * 0.02   * (1. - dist / 100.);
